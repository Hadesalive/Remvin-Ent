...
export function RemvinMinimalRenderer({ data, template, brandLogos = [] }: TemplateRendererProps) {
  ...
  // A4 page: 277mm available height (297mm - 25mm padding)
  // Header: ~70mm, Bill To: ~30mm, Table header: ~8mm, Totals: ~40mm, Footer: ~30mm
  // Used: ~178mm, Available: 277mm - 178mm = ~99mm → ~10-12 items per page
    ...
    <div style={{ marginBottom: '40px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '30px' }}>
        <div>
          {data.company.logo && (
            <img 
              ...
              alt="Company Logo" 
              style={{ height: '50px', marginBottom: '15px', objectFit: 'contain' }}
            />
          )}
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: '18px', fontWeight: '600', color: '#64748b', marginBottom: '15px', letterSpacing: '2px' }}>
          ...
          <div style={{ fontSize: '11px', color: '#64748b', lineHeight: '1.8' }}>
            ...
            <div>{formatDateValue(data.date)}</div>
            {!isQuote && <div>Due: {formatDateValue(data.dueDate)}</div>}
          </div>
        </div>
      </div>
    </div>
  ...
  const renderBillTo = () => (
    <div style={{ marginBottom: '40px' }}>
      ...
      <div style={{ fontSize: '13px', color: '#0f172a', lineHeight: '1.6' }}>
        <div style={{ fontWeight: '500', marginBottom: '4px' }}>{data.customer.name}</div>
        ...
        {adjustedPages.map((page, pageIndex) => {
          ...
          return (
            ...
            >
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ borderBottom: '2px solid #e2e8f0' }}>
                      <th style={{ padding: '10px 0', textAlign: 'left', fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Item</th>
                      <th style={{ padding: '10px 0', textAlign: 'center', fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Qty</th>
                      <th style={{ padding: '10px 0', textAlign: 'right', fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Rate</th>
                      <th style={{ padding: '10px 0', textAlign: 'right', fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Amount</th>
                    </tr>
                  </thead>
                  <tbody>
                    {pageItems.map((item, idx) => (
                      <tr key={item.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        <td style={{ padding: '12px 0', fontSize: '12px', color: '#0f172a' }}>
                          <div style={{ fontWeight: '400' }}>{item.description}</div>
                          {item.itemDescription && (
                            <div style={{ fontSize: '11px', color: '#94a3b8', marginTop: '2px' }}>{item.itemDescription}</div>
                          )}
                        </td>
                        <td style={{ padding: '12px 0', textAlign: 'center', fontSize: '12px', color: '#64748b' }}>{item.quantity}</td>
                        <td style={{ padding: '12px 0', textAlign: 'right', fontSize: '12px', color: '#64748b' }}>{formatCurrency(item.rate, currency)}</td>
                        <td style={{ padding: '12px 0', textAlign: 'right', fontSize: '12px', color: '#0f172a', fontWeight: '500' }}>{formatCurrency(item.amount, currency)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                  <div style={{ width: '250px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', fontSize: '12px' }}>
                      ...
                      <span style={{ color: '#0f172a' }}>{formatCurrency(subtotal, currency)}</span>
                    </div>
                    {data.discount > 0 && (
                      <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', fontSize: '12px' }}>
                        <span style={{ color: '#64748b' }}>Discount ({data.discount}%)</span>
                        <span style={{ color: '#0f172a' }}>-{formatCurrency(discountAmount, currency)}</span>
                      </div>
                    )}
                    {calculatedTaxes.map(tax => (
                      <div key={tax.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', fontSize: '12px' }}>
                        <span style={{ color: '#64748b' }}>{tax.name} ({tax.rate}%)</span>
                        <span style={{ color: '#0f172a' }}>{formatCurrency(tax.amount, currency)}</span>
                      </div>
                    ))}
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '15px 0', fontSize: '18px', fontWeight: '600', borderTop: '1px solid #e2e8f0', marginTop: '8px' }}>
              ...
              <div className="invoice-footer-section" style={{ marginTop: 'auto', paddingTop: '24px', borderTop: '1px solid #f1f5f9' }}>
                {/* Notes and Payment Due - side by side */}
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '20px', marginBottom: '16px' }}>
                  {/* Notes - Left side */}
                  <div style={{ flex: 1, fontSize: '11px', color: '#94a3b8', lineHeight: '1.5' }}>
                    {data.notes && (
